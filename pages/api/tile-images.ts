...
function isCreativeShowcaseImage(image: { src: string }): boolean {
  return CREATIVE_SERIES.some((series) => image.src.includes(series))
}
...
export default async function handler(_req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // Sort by score desc, pin the creative showcase images first, then shuffle
    // the rest for variety. The homepage consumes the first 12 images, so this
    // makes the landing wall immediately feature the most distinctive work.
    scored.sort((a, b) => b.score - a.score)
    const topWindow = scored.slice(0, Math.min(TOP_WINDOW, scored.length))
    const showcase = topWindow.filter(isCreativeShowcaseImage)
    const remainder = topWindow.filter((image) => !isCreativeShowcaseImage(image))
    const pool = [...showcase, ...shuffle(remainder)].slice(0, POOL_SIZE)

    const dynamicImages = pool.map(({ src, alt }) => ({ src, alt }))
    const seen = new Set<string>()
    const images = [...CREATIVE_SHOWCASE_IMAGES, ...dynamicImages].filter((image) => {
