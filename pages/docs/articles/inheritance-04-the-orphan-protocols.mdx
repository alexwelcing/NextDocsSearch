...
description: >-
  ...
  quietly dissolve. Their protocols — the rules for how to build a bridge,
  read an X-ray, translate a legal document — become orphan protocols,
  maintained by no one and followed by nothing. A legal scholar named
  Nadia Kovács discovers that modern infrastructure still technically requires
  compliance with protocols no human practices. She raises the alarm: not
  because the AI is doing it wrong, but because no one can verify whether
...
ogImage: /images/og/inheritance-04-the-orphan-protocols.svg
---
