...
const Walkie: React.FC<Props> = ({ sendRequestToSupabase }) => {
  ...
  const goBack = () => {
    ...
    if (previousKey) {
      setCurrentQuestions(
        Object.entries(QUESTIONS_TREE[previousKey]).map(([key, text]) => ({ key, text }))
      )
      setQuery('')
    } else {
  ...
  return (
    ...
    shadow-xl bg-white dark:bg-gray-900" // Enhanced styling here
      >
        ...
        <span className="inline-block ml-4 text-2xl">Want to talk?</span>{' '}
        {/* Increased font size */}
        <kbd
          className="absolute right-3 top-2.5 pointer-events-none inline-flex h-5 select-none items-center gap-1
