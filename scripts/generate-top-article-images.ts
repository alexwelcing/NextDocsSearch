...
async function writeImage(svg: string, outputPath: string) {
...
async function generateImages(article: Article) {
  ...
  const heroPath = path.join(heroDir, `${article.slug}.jpg`)
  const ogPath = path.join(ogDir, `${article.slug}.jpg`)
  ...
  await writeImage(heroSvg, heroPath)
  await writeImage(ogSvg, ogPath)

  updateArticleFrontmatter(article.filePath, `/images/og/${article.slug}.jpg`)
